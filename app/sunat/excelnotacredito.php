...
$documento
    ...
    ->setSubject('Reporte')
    ->setDescription('Reporte general de nota de crédito de la fecha ' . $fechaInicioFormato . ' al ' . $fechaFinalFormato)
    ->setKeywords('Reporte Nota de Crédito General')
    ->setCategory('Notas');
...
$documento->setActiveSheetIndex(0)
    ->setCellValue("A1", "REPORTE GENERAL DE NOTA DE CRÉDITO DE " .  $fechaInicioFormato . ' al ' . $fechaFinalFormato);
...
foreach ($ventas as $key => $value) {
    ...
    if (intval($value["Estado"]) !== 2 && $value["Xmlsunat"] === '0') {
        ...
        $documento->setActiveSheetIndex(0)
            ...
            ->setCellValue("M" . $cel, strval($value["ComprobateModificado"]));
    } else {
        $documento->getActiveSheet()->getStyle('A' . $cel . ':M' . $cel . '')->applyFromArray(array(
            'font'  => array(
                'bold'  =>  false,
        ...
        $documento->setActiveSheetIndex(0)
            ...
            ->setCellValue("K" . $cel, strval(round($value["Igv"], 2, PHP_ROUND_HALF_UP)))
            ->setCellValue("L" . $cel, strval(round($value["Base"] + $value["Igv"], 2, PHP_ROUND_HALF_UP)))
            ->setCellValue("M" . $cel, strtoupper("documento ha sido inutilizado"));
    }
